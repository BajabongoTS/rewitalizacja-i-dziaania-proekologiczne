...
export default function ContentSection({ id, badge, title, points, icon: Icon, index, prevSlideId, nextSlideId, prevSlideTitle, nextSlideTitle }: ContentSectionProps) {
  ...
  return (
    <section id={id} className={`min-h-screen flex flex-col ${isEven ? "" : "bg-section-alt"}`}>
      ...
      <div className="border-t border-border/40">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          {prevSlideId ? (
            <button
              onClick={() => scrollTo(prevSlideId)}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition group"
            >
              <ChevronLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
              <span className="hidden sm:inline truncate max-w-[200px]">{prevSlideTitle}</span>
              <span className="sm:hidden">Poprzedni</span>
            </button>
          ) : <div />}
          {nextSlideId ? (
            <button
              onClick={() => scrollTo(nextSlideId)}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition group"
            >
              <span className="hidden sm:inline truncate max-w-[200px]">{nextSlideTitle}</span>
